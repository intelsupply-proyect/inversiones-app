...
function ToastContainer() {
  ...
  const colors = {
    ...
    error:   { bg: "#fef2f2", border: "#fca5a5", color: "#dc2626", bar: "#ef4444" },
    warning: { bg: "#fffbeb", border: "#fcd34d", color: "#b45309", bar: "#f59e0b" },
    info:    { bg: "#eff6ff", border: "#93c5fd", color: "#1d4ed8", bar: "#3b82f6" },
  };
      ...
      {toasts.map(t => {
        ...
        return (
          <div key={t.id} style={{
            ...
            boxShadow: "0 8px 24px rgba(0,0,0,0.12)", animation: "toastIn 0.25s ease",
            position: "relative", overflow: "hidden",
          }}>
            <span style={{ fontSize: 18, flexShrink: 0, lineHeight: 1.3 }}>{icons[t.type]}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: c.color, lineHeight: 1.4 }}>{t.message}</div>
            </div>
            <button onClick={() => setToasts(prev => prev.filter(x => x.id !== t.id))}
              style={{ border: "none", background: "transparent", cursor: "pointer", fontSize: 16, color: c.color, opacity: 0.6, padding: 0, lineHeight: 1, flexShrink: 0 }}>×</button>
            {/* Progress bar */}
            <div style={{
              position: "absolute", bottom: 0, left: 0, height: 3, background: c.bar,
...
function Btn({ children, variant = "primary", loading: isLoading, ...props }) {
  ...
  return (
    <button
      ...
      style={{
        border: "none", borderRadius: 10, padding: "9px 18px", fontWeight: 600, fontSize: 13,
...
function ConfirmDialog({ open, title, message, confirmLabel = "Confirmar", confirmVariant = "danger", onConfirm, onCancel }) {
  if (!open) return null;
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.6)", zIndex: 1100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, backdropFilter: "blur(4px)" }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: 28, width: "100%", maxWidth: 400, boxShadow: "0 24px 80px rgba(0,0,0,0.25)" }}>
        <div style={{ fontSize: 40, textAlign: "center", marginBottom: 12 }}>⚠️</div>
        <div style={{ fontWeight: 700, fontSize: 16, color: "#0f172a", textAlign: "center", marginBottom: 8 }}>{title}</div>
        <div style={{ fontSize: 13, color: "#64748b", textAlign: "center", marginBottom: 24, lineHeight: 1.6 }}>{message}</div>
        <div style={{ display: "flex", gap: 10 }}>
          <Btn variant="secondary" onClick={onCancel} style={{ flex: 1 }}>Cancelar</Btn>
          <Btn variant={confirmVariant} onClick={onConfirm} style={{ flex: 1 }}>{confirmLabel}</Btn>
        </div>
      </div>
    </div>
  );
...
function useConfirm() {
  const [state, setState] = useState({ open: false, title: "", message: "", confirmLabel: "Confirmar", confirmVariant: "danger", resolve: null });
  const confirm = useCallback(({ title, message, confirmLabel = "Confirmar", confirmVariant = "danger" }) => {
    return new Promise(resolve => setState({ open: true, title, message, confirmLabel, confirmVariant, resolve }));
  }, []);
  ...
  const Dialog = () => (
    <ConfirmDialog open={state.open} title={state.title} message={state.message}
      confirmLabel={state.confirmLabel} confirmVariant={state.confirmVariant}
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0f172a 0%,#1e3a5f 60%,#1e40af 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, fontFamily: "'Inter','DM Sans','Segoe UI',sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 24, padding: 32, boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
          ...
          <Btn onClick={handleLogin} loading={loading} style={{ width: "100%", padding: 13, fontSize: 14 }}>
                ...
                <div key={o.id} style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #dcfce7", overflow: "hidden" }}>
                  <div style={{ background: "linear-gradient(135deg,#f0fdf4,#dcfce7)", padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
                    ...
                    <div style={{ display: "flex", gap: 12 }}>
                      <div style={{ textAlign: "center" }}><div style={{ fontSize: 11, color: "#64748b" }}>Capital</div><div style={{ fontSize: 15, fontWeight: 700, color: "#0f172a" }}>{fmt(totalInvertido)}</div></div>
                      <div style={{ textAlign: "center" }}><div style={{ fontSize: 11, color: "#64748b" }}>Interés/mes</div><div style={{ fontSize: 15, fontWeight: 700, color: "#16a34a" }}>{fmt(interesMensual)}</div></div>
                      <div style={{ textAlign: "center" }}><div style={{ fontSize: 11, color: "#64748b" }}>Días rest.</div><div style={{ fontSize: 15, fontWeight: 700, color: daysLeft(o.end_date) <= 30 ? "#dc2626" : "#f59e0b" }}>{daysLeft(o.end_date)}</div></div>
                    </div>
                  </div>
                  {parts.length > 0 && (
...
return urlData.publicUrl;
  ...
  function handleImgPaste(e) {
    ...
    for (let item of items) {
      if (item.type.startsWith("image/")) { const f = item.getAsFile(); setImgFile(f); setImgPreview(URL.createObjectURL(f)); break; }
    }
  }
  ...
  async function crearOrden(publicar = false) {
    ...
    setSaving(true);
    let imagen_url = null;
    if (imgFile) { try { imagen_url = await uploadImagen(imgFile); } catch(e) { console.error(e); } }
    const { error } = await supabase.from("investment_orders").insert({
      title: form.title, target_company: form.target_company, description: form.description,
      required_amount: parseFloat(form.required_amount), interest_rate: parseFloat(form.interest_rate) / 100,
      term_months: parseInt(form.term_months), minimum_amount: parseFloat(form.minimum_amount || 0),
      status: publicar ? "open" : "draft", created_by: profileId, code: "", imagen_url,
    });
    if (error) { setErr("Error al crear la orden. Intenta de nuevo."); setSaving(false); return; }
    toast(publicar ? "Orden publicada exitosamente" : "Borrador guardado", "success");
    setModalNueva(false);
    setForm({ title: "", target_company: "", description: "", required_amount: "", interest_rate: "", term_months: "", minimum_amount: "" });
    setImgFile(null); setImgPreview(null);
    loadOrdenes(); setSaving(false);
  }
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        {[["all","Todas"], ["draft","Borradores"], ["open","Abiertas"], ["funded","Fondeadas"], ["active","Activas"], ["completed","Completadas"]].map(([val, lbl]) => (
          <button key={val} onClick={() => setFiltro(val)}
            style={{ border: "none", borderRadius: 20, padding: "6px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", background: filtro === val ? "#0f172a" : "#f1f5f9", color: filtro === val ? "#fff" : "#64748b" }}>
            {lbl}
          </button>
        ))}
      </div>

      {loading ? <div style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>Cargando...</div> : isMobile ? (
        // ── MOBILE CARD VIEW ──
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {filtradas.map(o => (
            <div key={o.id} style={{ background: "#fff", borderRadius: 14, padding: 16, border: "1.5px solid #e2e8f0" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14, color: "#0f172a" }}>{o.title}</div>
                  <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2, fontFamily: "monospace" }}>{o.code}</div>
                </div>
                <Badge status={o.status} />
              </div>
              <div style={{ fontSize: 12, color: "#64748b", marginBottom: 10 }}>{o.target_company}</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 10 }}>
                <div style={{ background: "#f8fafc", borderRadius: 8, padding: "8px 10px" }}>
                  <div style={{ fontSize: 10, color: "#94a3b8" }}>MONTO</div>
                  <div style={{ fontSize: 13, fontWeight: 700 }}>{fmt(o.required_amount)}</div>
                </div>
                <div style={{ background: "#f0fdf4", borderRadius: 8, padding: "8px 10px" }}>
                  <div style={{ fontSize: 10, color: "#94a3b8" }}>TASA</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#16a34a" }}>{(parseFloat(o.interest_rate) * 100).toFixed(1)}%</div>
                </div>
                <div style={{ background: "#f8fafc", borderRadius: 8, padding: "8px 10px" }}>
                  <div style={{ fontSize: 10, color: "#94a3b8" }}>PLAZO</div>
                  <div style={{ fontSize: 13, fontWeight: 700 }}>{o.term_months}m</div>
                </div>
              </div>
              <ProgressBar value={parseFloat(o.funded_amount || 0)} max={parseFloat(o.required_amount)} />
              <div style={{ marginTop: 10 }}>
                <Btn variant="info" style={{ width: "100%", fontSize: 12 }} onClick={() => setModalDetalle(o)}>Ver detalle</Btn>
              </div>
            </div>
          ))}
          {filtradas.length === 0 && <div style={{ padding: 40, textAlign: "center", color: "#94a3b8", background: "#fff", borderRadius: 16 }}>No hay órdenes</div>}
        </div>
      ) : (
        // ── DESKTOP TABLE VIEW ──
        <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", overflow: "hidden" }}>
            ...
            <tbody>
              {filtradas.map((o, i) => (
                <tr key={o.id} style={{ borderTop: "1px solid #f1f5f9", background: i % 2 === 0 ? "#fff" : "#fafbfc" }}>
                  ...
                  <td style={{ padding: "11px 14px", minWidth: 120 }}><ProgressBar value={parseFloat(o.funded_amount || 0)} max={parseFloat(o.required_amount)} /></td>
              ...
              {filtradas.length === 0 && <tr><td colSpan={9} style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>No hay órdenes</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      ...
      <Modal open={modalNueva} onClose={() => { setModalNueva(false); setErr(""); }} title="Nueva orden de inversión">
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div onClick={() => imgRef.current?.click()} onPaste={handleImgPaste} tabIndex={0}
            ...
            {imgPreview
              ...
              : <div><div style={{ fontSize: 28, marginBottom: 6 }}>🖼️</div><div style={{ fontSize: 13, color: "#64748b", fontWeight: 500 }}>Clic para buscar imagen</div><div style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>o pega una captura con Ctrl+V</div></div>
            }
          </div>
          <input ref={imgRef} type="file" accept="image/*" onChange={handleImgFile} style={{ display: "none" }} />
          {imgPreview && <button onClick={() => { setImgFile(null); setImgPreview(null); }} style={{ marginTop: 6, border: "none", background: "#fef2f2", color: "#dc2626", borderRadius: 8, padding: "4px 12px", fontSize: 11, cursor: "pointer", fontWeight: 600 }}>✕ Quitar imagen</button>}
        </div>
        {err && <div style={{ color: "#dc2626", fontSize: 12, marginBottom: 12, padding: "8px 12px", background: "#fef2f2", borderRadius: 8 }}>⚠️ {err}</div>}
        <div style={{ display: "flex", gap: 8 }}>
          <Btn variant="secondary" onClick={() => setModalNueva(false)} style={{ flex: 1 }}>Cancelar</Btn>
          <Btn variant="secondary" onClick={() => crearOrden(false)} loading={saving} style={{ flex: 1 }}>Guardar borrador</Btn>
          <Btn onClick={() => crearOrden(true)} loading={saving} style={{ flex: 1 }}>Publicar ahora</Btn>
        </div>
      </Modal>
  ...
  try {
    ...
    const total = totalCalculado !== undefined ? totalCalculado : parseFloat(part.amount) + interest;
      ...
      await supabase.from("earnings").insert({
        participation_id: part.participation_id, investor_id: part.investor_id,
        principal: parseFloat(part.amount), interest_earned: interest,
        type: "maturity", payout_date: new Date().toISOString().split("T")[0], status: "paid",
      });
      await supabase.from("participations").update({ status: "completed" }).eq("id", part.participation_id);
      toast(`Pago procesado a ${part.investor_name} — ${fmt(total)}`, "success");
      setModalPago(null);
      loadParticipaciones();
      onReload();
    } catch (e) {
      toast("Error al procesar el pago. Intenta de nuevo.", "error");
    } finally {
...
async function cerrarOrdenAnticipado() {
    ...
    toast("Orden cerrada anticipadamente", "warning");
    onReload();
  }

  async function eliminarOrden() {
    const ok = await confirm({ title: `¿Eliminar la orden "${ordenActual.title}"?`, message: "Esta acción no se puede deshacer y eliminará todos los datos relacionados.", confirmLabel: "Sí, eliminar", confirmVariant: "danger" });
    if (!ok) return;
    const { error } = await supabase.from("investment_orders").delete().eq("id", ordenActual.id);
    if (error) { toast("Error al eliminar la orden.", "error"); return; }
    toast("Orden eliminada correctamente", "success");
    onClose(); onReload();
  }
  ...
  return (
    <Modal open={true} onClose={onClose} title={`Orden: ${ordenActual.code}`} maxWidth={700}>
      ...
      {ordenActual.imagen_url && (
        <img src={ordenActual.imagen_url} alt={ordenActual.title} style={{ width: "100%", height: 180, objectFit: "cover", borderRadius: 12, marginBottom: 16 }} />
      )}
...
function AdminInversionistas() {
  ...
  async function registrarDeposito(tipo = "deposit") {
    if (!deposito.monto || !deposito.descripcion) { toast("Completa el monto y la descripción.", "warning"); return; }
    setSaving(true);
    ...
    const { error } = await supabase.rpc("record_capital_movement", {
      p_investor_id: modalDeposito.investor_id, p_amount: monto,
      p_type: tipo === "deposit" ? "deposit" : "withdrawal",
      p_description: deposito.descripcion,
    });
    if (error) { toast("Error al registrar el movimiento.", "error"); setSaving(false); return; }
    toast(tipo === "deposit" ? `Depósito de ${fmt(parseFloat(deposito.monto))} registrado` : `Retiro de ${fmt(parseFloat(deposito.monto))} registrado`, "success");
    setModalDeposito(null); setDeposito({ monto: "", descripcion: "" });
    loadInversores(); setSaving(false);
  }
    ...
    <div>
          ...
          {inversores.map(inv => (
            <div key={inv.investor_id} style={{ background: "#fff", borderRadius: 16, padding: 20, border: "1.5px solid #e2e8f0" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                ...
                <div style={{ width: 40, height: 40, borderRadius: 12, background: "#f5f3ff", color: "#7c3aed", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 16 }}>{inv.full_name?.[0]}</div>
              </div>
              ...
              {inv.banco && (
                <div style={{ background: "#f8fafc", borderRadius: 10, padding: "8px 12px", marginBottom: 10, fontSize: 12 }}>
                  ...
                  {inv.cuenta_bancaria && <><span style={{ color: "#94a3b8", marginLeft: 8 }}>Cuenta: </span><strong>****{inv.cuenta_bancaria.slice(-4)}</strong></>}
                </div>
              )}
      ...
      <Modal open={!!modalDeposito} onClose={() => setModalDeposito(null)} title={`Registrar movimiento — ${modalDeposito?.full_name}`} maxWidth={420}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Btn variant="danger" loading={saving} onClick={() => registrarDeposito("withdrawal")} style={{ flex: 1 }}>↓ Retiro</Btn>
          <Btn variant="success" loading={saving} onClick={() => registrarDeposito("deposit")} style={{ flex: 1 }}>↑ Depósito</Btn>
        </div>
      </Modal>
...
function ModalDetalleInversor({ inv, onClose }) {
  ...
  const [ajuste, setAjuste] = useState({ balance: inv.available_balance, motivo: "" });
  ...
  const totalRetirado = movimientos.filter(m => m.type === "withdrawal" && !m.description?.includes("Interés mensual retirado")).reduce((a, b) => a + Math.abs(parseFloat(b.amount || 0)), 0);
  ...
  async function guardarAjuste() {
    if (!ajuste.motivo) { toast("Escribe el motivo del ajuste.", "warning"); return; }
    setSavingAjuste(true);
    const diff = parseFloat(ajuste.balance) - parseFloat(inv.available_balance);
    const { error } = await supabase.from("profiles").update({ available_balance: parseFloat(ajuste.balance) }).eq("id", inv.investor_id);
    if (error) { toast("Error al aplicar el ajuste.", "error"); setSavingAjuste(false); return; }
    await supabase.from("capital_movements").insert({
      investor_id: inv.investor_id, amount: diff, type: "admin_adjustment",
      description: `Ajuste manual: ${ajuste.motivo}`,
      balance_before: parseFloat(inv.available_balance), balance_after: parseFloat(ajuste.balance),
    });
    toast("Ajuste de balance aplicado", "success");
    setModalAjuste(false); setSavingAjuste(false); loadData();
  }

  async function guardarBanco() {
    setSavingBanco(true);
    const { error } = await supabase.from("profiles").update({ banco, cuenta_bancaria: cuenta, phone }).eq("id", inv.investor_id);
    if (error) { toast("Error al guardar los datos bancarios.", "error"); setSavingBanco(false); return; }
    toast("Datos bancarios actualizados", "success");
    setEditBanco(false); setSavingBanco(false);
  }
    ...
    <Modal open={true} onClose={onClose} title={inv.full_name} maxWidth={720}>
      ...
      <div style={{ background: "#f8fafc", borderRadius: 12, padding: "12px 16px", marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: editBanco ? 12 : 0 }}>
          ...
          <Btn variant="secondary" style={{ padding: "4px 10px", fontSize: 11 }} onClick={() => setEditBanco(p => !p)}>{editBanco ? "Cancelar" : "Editar"}</Btn>
        </div>
        {editBanco ? (
          <div>
            ...
            <Btn onClick={guardarBanco} loading={savingBanco} style={{ width: "100%" }}>Guardar</Btn>
          </div>
        ) : (
          <div style={{ fontSize: 13, marginTop: 4 }}>
            {inv.banco ? <span><strong>{inv.banco}</strong> — Cuenta: <strong>{inv.cuenta_bancaria ? `****${inv.cuenta_bancaria.slice(-4)}` : "—"}</strong></span>
      ...
      {loading ? <div style={{ padding: 20, textAlign: "center", color: "#94a3b8" }}>Cargando...</div> : (
        <>
              ...
              {participaciones.map(p => (
                <div key={p.participation_id} style={{ background: "#f8fafc", borderRadius: 12, padding: "14px 16px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div><div style={{ fontWeight: 600, fontSize: 13 }}>{p.order_title}</div><div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>{p.target_company}</div></div>
                    <Badge status={p.status} />
                  </div>
          ...
          {tab === "pagos" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {pagos.length === 0 ? (
                <div style={{ padding: 30, textAlign: "center", color: "#94a3b8" }}><div style={{ fontSize: 32, marginBottom: 8 }}>📅</div><div>Sin pagos mensuales registrados aún</div></div>
              ) : pagos.map(p => (
                <div key={p.id} style={{ background: "#f8fafc", borderRadius: 12, padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    {p.comprobante_url
                      ? <a href={p.comprobante_url} target="_blank" rel="noreferrer"><Btn variant="info" style={{ padding: "5px 10px", fontSize: 11 }}>Ver 🧾</Btn></a>
                      : <Btn variant="warning" style={{ padding: "5px 10px", fontSize: 11 }} onClick={() => { setModalComprobante(p); setComprobanteFile(null); setComprobantePreview(null); }}>Subir 📎</Btn>
                    }
...
{tab === "ganancias" && (() => {
      ...
      <Modal open={modalAjuste} onClose={() => setModalAjuste(false)} title="Ajustar balance" maxWidth={400}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Btn variant="warning" loading={savingAjuste} onClick={guardarAjuste} disabled={!ajuste.motivo} style={{ flex: 1 }}>Aplicar ajuste</Btn>
        </div>
      </Modal>
...
function AdminPagosMensuales() {
  ...
  function handleImgFile(e) { const f = e.target.files[0]; if (!f) return; setImgFile(f); setImgPreview(URL.createObjectURL(f)); }
  function handlePaste(e) {
    const items = e.clipboardData?.items; if (!items) return;
    for (let item of items) { if (item.type.startsWith("image/")) { const f = item.getAsFile(); setImgFile(f); setImgPreview(URL.createObjectURL(f)); break; } }
  }

  async function crearPago() {
    if (!form.participation_id || !form.monto_interes) { toast("Selecciona la participación e ingresa el monto.", "warning"); return; }
    setSaving(true);
  ...
  comprobante_url = urlData.publicUrl;
    ...
    const { error } = await supabase.from("pagos_mensuales").insert({
      ...
      mes: parseInt(form.mes), anio: parseInt(form.anio), monto_interes: parseFloat(form.monto_interes),
      notas: form.notas, comprobante_url,
      status: comprobante_url ? "pagado" : "pendiente",
      fecha_pago: comprobante_url ? new Date().toISOString().split("T")[0] : null,
    });
    if (error) { toast("Error al registrar el pago.", "error"); setSaving(false); return; }
    toast(`Pago mensual de ${fmt(parseFloat(form.monto_interes))} registrado`, "success");
    setModalNuevo(false);
    ...
    <div>
      ...
      {loading ? <div style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>Cargando...</div> : isMobile ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {pagosFiltrados.map(p => (
            <div key={p.id} style={{ background: "#fff", borderRadius: 14, padding: 16, border: "1.5px solid #e2e8f0" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 13 }}>{p.profiles?.full_name}</div>
                  <div style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>{p.investment_orders?.title}</div>
                </div>
                <Badge status={p.status === "pagado" ? "paid" : "pending"} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                <div style={{ background: "#f0fdf4", borderRadius: 8, padding: "8px 10px" }}>
                  <div style={{ fontSize: 10, color: "#94a3b8" }}>INTERÉS</div>
                  <div style={{ fontSize: 15, fontWeight: 800, color: "#16a34a" }}>{fmt(p.monto_interes)}</div>
                </div>
                <div style={{ background: "#f8fafc", borderRadius: 8, padding: "8px 10px" }}>
                  <div style={{ fontSize: 10, color: "#94a3b8" }}>MES / AÑO</div>
                  <div style={{ fontSize: 13, fontWeight: 700 }}>{mesesNombre[p.mes - 1].substring(0,3)} {p.anio}</div>
                </div>
              </div>
              {p.fecha_pago && <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 8 }}>Pagado: {fmtDate(p.fecha_pago)}</div>}
              {p.comprobante_url && (
                <a href={p.comprobante_url} target="_blank" rel="noreferrer">
                  <Btn variant="info" style={{ width: "100%", fontSize: 12 }}>Ver comprobante 🧾</Btn>
                </a>
              )}
            </div>
          ))}
          {pagosFiltrados.length === 0 && <div style={{ padding: 40, textAlign: "center", color: "#94a3b8", background: "#fff", borderRadius: 16 }}>Sin pagos registrados</div>}
        </div>
      ) : (
        // ── DESKTOP TABLE VIEW ──
        <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              ...
              {pagosFiltrados.length === 0 && <tr><td colSpan={7} style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>Sin pagos registrados</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      ...
      <Modal open={modalNuevo} onClose={() => setModalNuevo(false)} title="Registrar pago mensual" maxWidth={460}>
        <Sel label="Inversionista / Participación *" value={form.participation_id} onChange={e => {
          ...
          {participaciones.map(p => <option key={p.participation_id} value={p.participation_id}>{p.investor_name} — {p.order_title} ({fmt(p.amount)})</option>)}
        </Sel>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div onClick={() => imgRef.current?.click()} onPaste={handlePaste} tabIndex={0}
            ...
            {imgPreview
              ...
              : <div><div style={{ fontSize: 24, marginBottom: 4 }}>📎</div><div style={{ fontSize: 12, color: "#64748b" }}>Clic para buscar imagen</div><div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>o pega con Ctrl+V</div></div>
            }
          </div>
          <input ref={imgRef} type="file" accept="image/*,.pdf" onChange={handleImgFile} style={{ display: "none" }} />
          {imgPreview && <button onClick={() => { setImgFile(null); setImgPreview(null); }} style={{ marginTop: 5, border: "none", background: "#fef2f2", color: "#dc2626", borderRadius: 8, padding: "3px 10px", fontSize: 11, cursor: "pointer" }}>✕ Quitar</button>}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <Btn variant="secondary" onClick={() => setModalNuevo(false)} style={{ flex: 1 }}>Cancelar</Btn>
          <Btn loading={saving} onClick={crearPago} style={{ flex: 1 }}>Registrar pago</Btn>
        </div>
      </Modal>
    </div>
  );
...
function AdminVencimientos() {
  ...
  async function devolver(p) {
    ...
    const ok = await confirm({ title: `¿Procesar devolución para ${p.investor_name}?`, message: `Se devolverá capital + intereses por un total de ${fmt(parseFloat(p.amount) * (1 + parseFloat(p.interest_rate)))}.`, confirmLabel: "Sí, devolver", confirmVariant: "success" });
    if (!ok) return;
    ...
    try {
      ...
      await supabase.rpc("record_capital_movement", {
        p_investor_id: p.investor_id, p_amount: total, p_type: "investment_return",
        p_description: `Devolución capital + intereses — ${p.order_title}`, p_participation_id: p.participation_id,
      });
      await supabase.from("earnings").insert({
        participation_id: p.participation_id, investor_id: p.investor_id, principal: parseFloat(p.amount),
        interest_earned: interest, type: "maturity", payout_date: new Date().toISOString().split("T")[0], status: "paid",
      });
      await supabase.from("participations").update({ status: "completed" }).eq("id", p.participation_id);
      toast(`Devolución de ${fmt(total)} procesada a ${p.investor_name}`, "success");
      loadData();
    } catch (e) {
      toast("Error al procesar la devolución.", "error");
    } finally {
      setProcesando(null);
    }
  }
      ...
      {participaciones.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {participaciones.map(p => {
            ...
            return (
              <div key={p.participation_id} style={{ background: "#fff", borderRadius: 14, padding: 16, border: `1.5px solid ${dias <= 0 ? "#fecaca" : dias <= 7 ? "#fde68a" : "#e2e8f0"}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14 }}>{p.investor_name}</div>
                    <div style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>{p.order_title}</div>
                  </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
                  <div style={{ background: "#f8fafc", borderRadius: 8, padding: "8px 10px" }}><div style={{ fontSize: 10, color: "#94a3b8" }}>CAPITAL</div><div style={{ fontSize: 13, fontWeight: 700 }}>{fmt(p.amount)}</div></div>
                  <div style={{ background: "#f0fdf4", borderRadius: 8, padding: "8px 10px" }}><div style={{ fontSize: 10, color: "#94a3b8" }}>GANANCIA</div><div style={{ fontSize: 13, fontWeight: 700, color: "#16a34a" }}>{fmt(ganancia)}</div></div>
                  <div style={{ background: "#f5f3ff", borderRadius: 8, padding: "8px 10px" }}><div style={{ fontSize: 10, color: "#94a3b8" }}>TOTAL</div><div style={{ fontSize: 13, fontWeight: 700, color: "#7c3aed" }}>{fmt(total)}</div></div>
                </div>
                <div style={{ fontSize: 12, color: "#64748b", marginBottom: 10 }}>Vence: {fmtDate(p.end_date)}</div>
                <Btn variant="success" style={{ width: "100%" }} loading={procesando === p.participation_id} onClick={() => devolver(p)}>
                  💰 Devolver {fmt(total)}
                </Btn>
              </div>
            );
              ...
              {participaciones.map((p, i) => {
                ...
                return (
                  <tr key={p.participation_id} style={{ borderTop: "1px solid #f1f5f9", background: i % 2 === 0 ? "#fff" : "#fafbfc" }}>
                    ...
                    <td style={{ padding: "11px 14px" }}>
                      <Btn variant="success" style={{ padding: "5px 12px", fontSize: 11 }} loading={procesando === p.participation_id} onClick={() => devolver(p)}>Devolver</Btn>
                    </td>
                  </tr>
                );
              })}
            ...
            {inversiones.map(p => (
              <div key={p.participation_id} style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #e2e8f0", overflow: "hidden" }}>
                {p.imagen_url
                  ...
                  : <div style={{ width: "100%", height: 80, background: "linear-gradient(135deg,#0f172a,#1e3a5f)", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontSize: 28 }}>📈</span></div>
                }
                <div style={{ padding: 18 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                    <div><div style={{ fontWeight: 700, fontSize: 14, color: "#0f172a" }}>{p.order_title}</div><div style={{ fontSize: 12, color: "#94a3b8", marginTop: 2 }}>{p.target_company}</div></div>
                    <Badge status={p.status} />
                  </div>
...
function PortalOportunidades({ profileId, onParticipacionExitosa }) {
  ...
  async function participar() {
    ...
    try {
      const { data: part, error } = await supabase.from("participations").insert({
        investor_id: profileId, order_id: modalParticipar.id,
        amount: montoNum, interest_rate: parseFloat(modalParticipar.interest_rate), status: "pending",
      }).select().single();
      if (error) throw error;
      const { error: moveErr } = await supabase.rpc("record_capital_movement", {
        p_investor_id: profileId, p_amount: -montoNum, p_type: "investment_lock",
      ...
      toast("No se pudo registrar la participación.", "error");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        ...
        <div style={{ background: "#f5f3ff", borderRadius: 12, padding: "8px 16px", fontSize: 13, fontWeight: 700, color: "#7c3aed" }}>Balance disponible: {fmt(balanceDisponible)}</div>
      </div>
      {ordenes.length === 0 ? (
        <div style={{ background: "#fff", borderRadius: 16, padding: 60, textAlign: "center", border: "1.5px solid #e2e8f0" }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>🔍</div>
          ...
          {ordenes.map(o => (
            <div key={o.id} style={{ background: "#fff", borderRadius: 16, overflow: "hidden", border: "1.5px solid #e2e8f0" }}>
              <div style={{ width: "100%", height: 120, background: "linear-gradient(135deg,#0f172a,#1e3a5f)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
                <div style={{ textAlign: "center" }}><div style={{ fontSize: 32, marginBottom: 4 }}>📈</div><div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)" }}>{o.target_company}</div></div>
                {o.imagen_url && (
                  <a href={o.imagen_url} target="_blank" rel="noreferrer" style={{ position: "absolute", bottom: 10, right: 10, background: "rgba(255,255,255,0.15)", backdropFilter: "blur(4px)", color: "#fff", borderRadius: 8, padding: "5px 12px", fontSize: 11, fontWeight: 600, textDecoration: "none", border: "1px solid rgba(255,255,255,0.2)" }}>
                    📎 Ver documento
                  </a>
                )}
              </div>
              <div style={{ padding: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                  <div><div style={{ fontWeight: 700, fontSize: 15, color: "#0f172a" }}>{o.title}</div><div style={{ fontSize: 12, color: "#94a3b8", marginTop: 3 }}>{o.target_company}</div></div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
                  <div style={{ background: "#f8fafc", borderRadius: 10, padding: "10px 12px" }}><div style={{ fontSize: 10, color: "#94a3b8", marginBottom: 3 }}>PLAZO</div><div style={{ fontSize: 14, fontWeight: 700 }}>{o.term_months} meses</div></div>
                  <div style={{ background: "#f8fafc", borderRadius: 10, padding: "10px 12px" }}><div style={{ fontSize: 10, color: "#94a3b8", marginBottom: 3 }}>MÍNIMO</div><div style={{ fontSize: 14, fontWeight: 700 }}>{fmt(o.minimum_amount || 0)}</div></div>
                </div>
                <div style={{ marginBottom: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
  ...
  setModalParticipar(o); setMonto(minimoEfectivo.toString()); setErr(""); 
      ...
      <Modal open={!!modalParticipar} onClose={() => { if (!saving) setModalParticipar(null); }} title={`Participar — ${modalParticipar?.title}`} maxWidth={440}>
        {exito ? (
          ...
          <>
            ...
            {err && <div style={{ color: "#dc2626", fontSize: 12, marginBottom: 12, padding: "8px 12px", background: "#fef2f2", borderRadius: 8 }}>⚠️ {err}</div>}
            <div style={{ display: "flex", gap: 8 }}>
              <Btn variant="secondary" onClick={() => setModalParticipar(null)} disabled={saving} style={{ flex: 1 }}>Cancelar</Btn>
              <Btn loading={saving} onClick={participar} style={{ flex: 1, background: "#7c3aed" }}>
    ...
    <div>
      ...
      {historial.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {historial.map(p => (
            <div key={p.participation_id} style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #e2e8f0", overflow: "hidden" }}>
              {p.imagen_url ? <img src={p.imagen_url} alt={p.order_title} style={{ width: "100%", height: 100, objectFit: "cover" }} />
                : <div style={{ width: "100%", height: 70, background: "linear-gradient(135deg,#0f172a,#1e3a5f)", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontSize: 24 }}>📂</span></div>}
              <div style={{ padding: 18 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                  <div><div style={{ fontWeight: 700, fontSize: 14 }}>{p.order_title}</div><div style={{ fontSize: 12, color: "#94a3b8", marginTop: 2 }}>{p.target_company}</div></div>
                  <Badge status={p.status} />
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }}>
                  <div style={{ background: "#f8fafc", borderRadius: 10, padding: "10px 12px" }}><div style={{ fontSize: 10, color: "#94a3b8", marginBottom: 3 }}>Capital invertido</div><div style={{ fontSize: 14, fontWeight: 700 }}>{fmt(p.amount)}</div></div>
                  <div style={{ background: "#f0fdf4", borderRadius: 10, padding: "10px 12px" }}><div style={{ fontSize: 10, color: "#16a34a", marginBottom: 3 }}>Ganancia recibida</div><div style={{ fontSize: 14, fontWeight: 700, color: "#16a34a" }}>{fmt(p.actual_earnings || p.projected_earnings)}</div></div>
                  <div style={{ background: "#f8fafc", borderRadius: 10, padding: "10px 12px" }}><div style={{ fontSize: 10, color: "#94a3b8", marginBottom: 3 }}>Venció</div><div style={{ fontSize: 14, fontWeight: 700 }}>{fmtDate(p.end_date)}</div></div>
                </div>
              </div>
            </div>
          ))}
...
function Calendario({ eventos }) {
  ...
  const eventosMes = eventos.filter(e => { const d = new Date(e.fecha); return d.getMonth() === mesActual && d.getFullYear() === anioActual; });
  const eventosPorDia = {};
  eventosMes.forEach(e => { const dia = new Date(e.fecha).getDate(); if (!eventosPorDia[dia]) eventosPorDia[dia] = []; eventosPorDia[dia].push(e); });
  function prevMes() { if (mesActual === 0) { setMesActual(11); setAnioActual(a => a - 1); } else setMesActual(m => m - 1); }
  function nextMes() { if (mesActual === 11) { setMesActual(0); setAnioActual(a => a + 1); } else setMesActual(m => m + 1); }
  const celdas = [];
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", overflow: "hidden" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", background: "#f8fafc" }}>
        {diasSemana.map(d => <div key={d} style={{ padding: "8px 0", textAlign: "center", fontSize: 11, fontWeight: 600, color: "#94a3b8" }}>{d}</div>)}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 1, background: "#f1f5f9", padding: 1 }}>
        {celdas.map((dia, i) => {
          ...
          return (
            <div key={i} style={{ background: "#fff", minHeight: 52, padding: 4 }}>
              <div style={{ width: 24, height: 24, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", background: esHoy ? "#0f172a" : "transparent", color: esHoy ? "#fff" : "#374151", fontSize: 12, fontWeight: esHoy ? 700 : 400, marginBottom: 2 }}>{dia}</div>
              {evs.map((e, j) => (
                <div key={j} title={e.label} style={{ fontSize: isMobile ? 7 : 9, fontWeight: 600, borderRadius: 4, padding: "1px 3px", marginBottom: 1, background: e.pagado ? "#dcfce7" : e.alerta ? "#fee2e2" : "#dbeafe", color: e.pagado ? "#15803d" : e.alerta ? "#991b1b" : "#1d4ed8", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
  ...
  async function loadData() {
    ...
    (parts || []).forEach(p => {
      ...
      for (let m = 1; m <= (p.term_months || 1); m++) {
        const fechaPago = new Date(start); fechaPago.setMonth(fechaPago.getMonth() + m);
        const mes = fechaPago.getMonth() + 1; const anio = fechaPago.getFullYear();
        const diasRestantes = Math.ceil((fechaPago - hoy) / (1000 * 60 * 60 * 24));
        const pagoExistente = (pagos || []).find(pg => pg.participation_id === p.participation_id && pg.mes === mes && pg.anio === anio);
        evs.push({ fecha: fechaPago.toISOString().split("T")[0], label: `${p.investor_name} — ${p.order_title}`, monto: interesMensual, pagado: pagoExistente?.status === "pagado", alerta: diasRestantes <= 3 && diasRestantes >= 0 && !pagoExistente, icono: pagoExistente?.status === "pagado" ? "✅" : diasRestantes <= 3 ? "⚠️" : "💰" });
      }
    });
    ...
    <div>
      ...
      {alertas.length > 0 && (
        <div style={{ background: "#fef2f2", border: "1.5px solid #fecaca", borderRadius: 14, padding: "14px 18px", marginBottom: 20 }}>
          ...
          {alertas.map((e, i) => <div key={i} style={{ fontSize: 13, color: "#374151", marginBottom: 4 }}>• {e.label} — {fmt(e.monto)} — {fmtDate(e.fecha)}</div>)}
        </div>
      )}
  ...
  async function loadData() {
    ...
    (parts || []).forEach(p => {
      ...
      for (let m = 1; m <= (p.term_months || 1); m++) {
        ...
        evs.push({ fecha: fechaPago.toISOString().split("T")[0], label: p.order_title, monto: interesMensual, pagado: pagoExistente?.status === "pagado", alerta: diasRestantes <= 3 && diasRestantes >= 0 && !pagoExistente, icono: pagoExistente?.status === "pagado" ? "✅" : "💰" });
      }
    });
    setEventos(evs); setLoading(false);
  }
...
function FormOC({ titulo, form, setForm, clientes, saving, docFile, setDocFile, docPreview, setDocPreview, analizando, onAnalizar, onGuardar, onCerrar }) {
  ...
  function handleClienteChange(clienteId) {
    ...
    const venc = form.fecha_facturacion ? autoVencimiento(form.fecha_facturacion, cl?.plazo_dias || 90) : "";
  ...
  function handleDocFile(e) { const f = e.target.files[0]; if (!f) return; setDocFile(f); setDocPreview(URL.createObjectURL(f)); }
  function handleDocPaste(e) {
    const items = e.clipboardData?.items; if (!items) return;
    for (let item of items) { if (item.type.startsWith("image/")) { const f = item.getAsFile(); setDocFile(f); setDocPreview(URL.createObjectURL(f)); break; } }
  }
  return (
    <Modal open={true} onClose={onCerrar} title={titulo} maxWidth={600}>
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#64748b", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.05em" }}>Documento OC (imagen o PDF)</div>
        <div onClick={() => docRef.current?.click()} onPaste={handleDocPaste} tabIndex={0}
          ...
          onMouseLeave={e => e.currentTarget.style.borderColor = "#e2e8f0"}>
          {docPreview ? <img src={docPreview} alt="doc" style={{ maxHeight: 140, maxWidth: "100%", borderRadius: 8, objectFit: "contain" }} />
            : <div><div style={{ fontSize: 22, marginBottom: 4 }}>📎</div><div style={{ fontSize: 12, color: "#64748b" }}>Clic para buscar archivo o pega imagen con Ctrl+V</div><div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>PNG, JPG, PDF</div></div>}
        </div>
        <input ref={docRef} type="file" accept="image/*,.pdf" onChange={handleDocFile} style={{ display: "none" }} />
        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          {docPreview && <button onClick={() => { setDocFile(null); setDocPreview(null); }} style={{ border: "none", background: "#fef2f2", color: "#dc2626", borderRadius: 8, padding: "4px 10px", fontSize: 11, cursor: "pointer", fontWeight: 600 }}>✕ Quitar</button>}
          {docFile && <Btn loading={analizando} onClick={onAnalizar} style={{ fontSize: 11, padding: "5px 14px", background: "#7c3aed", color: "#fff" }}>{analizando ? "Analizando con IA..." : "✨ Leer con IA y llenar campos"}</Btn>}
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <Btn variant="secondary" onClick={onCerrar} style={{ flex: 1 }}>Cancelar</Btn>
        <Btn loading={saving} onClick={onGuardar} style={{ flex: 1 }}>Guardar OC</Btn>
      </div>
    </Modal>
  );
}
...
function AdminOCxCobrar({ profileId }) {
  ...
  const [form, setForm] = useState({ cliente_id: "", numero_oc: "", descripcion: "", fecha_facturacion: "", monto_total: "", fecha_vencimiento: "", fecha_pago_esperado: "", fecha_pago_real: "", notas: "" });
  ...
  function abrirCrearOrden(oc) {
    setFormOrden({ title: `OC #${oc.numero_oc} — ${oc.clientes_oc?.nombre || ""}`, target_company: oc.clientes_oc?.nombre || "", description: oc.descripcion || "", required_amount: "", interest_rate: "", term_months: "", minimum_amount: "" });
    setImgOrden(null); setImgOrdenPreview(null); setModalCrearOrden(oc);
  }

  async function guardarOrdenDesdeOC() {
    if (!formOrden.required_amount || !formOrden.interest_rate || !formOrden.term_months) { toast("Completa el monto, tasa y plazo.", "warning"); return; }
    setSavingOrden(true);
    ...
    const { error } = await supabase.from("investment_orders").insert({
      title: formOrden.title, target_company: formOrden.target_company, description: formOrden.description,
      required_amount: parseFloat(formOrden.required_amount), funded_amount: 0,
      interest_rate: parseFloat(formOrden.interest_rate) / 100, term_months: parseInt(formOrden.term_months),
  ...
  async function analizarDocumento() {
    ...
    setAnalizando(true);
    try {
      const base64 = await new Promise((res, rej) => { const r = new FileReader(); r.onload = () => res(r.result.split(",")[1]); r.onerror = rej; r.readAsDataURL(docFile); });
      ...
      setForm(p => ({ ...p, numero_oc: parsed.numero_oc || p.numero_oc, descripcion: parsed.descripcion || p.descripcion, monto_total: parsed.monto_total || p.monto_total, fecha_facturacion: parsed.fecha_facturacion || p.fecha_facturacion, notas: parsed.notas || p.notas }));
      toast("Campos llenados automáticamente con IA", "success");
    } catch (e) {
      console.error("Error analizando documento:", e);
      toast("No se pudo analizar el documento.", "error");
    }
    setAnalizando(false);
  }

  async function guardarOC(editando = false) {
    if (!form.cliente_id || !form.numero_oc || !form.monto_total) { toast("Completa cliente, N° OC y monto.", "warning"); return; }
    setSaving(true);
    ...
    const payload = { cliente_id: form.cliente_id, numero_oc: form.numero_oc, descripcion: form.descripcion, fecha_facturacion: form.fecha_facturacion || null, monto_total: parseFloat(form.monto_total), fecha_vencimiento: form.fecha_vencimiento || null, fecha_pago_esperado: form.fecha_pago_esperado || null, fecha_pago_real: form.fecha_pago_real || null, notas: form.notas, estado: form.fecha_pago_real ? "pagado" : "pendiente", documento_url };
    if (editando) {
      const { error } = await supabase.from("ordenes_por_cobrar").update(payload).eq("id", modalEditar.id);
      if (error) { toast("Error al actualizar la OC.", "error"); setSaving(false); return; }
      toast("OC actualizada correctamente", "success");
      setModalEditar(null);
    } else {
      const { error } = await supabase.from("ordenes_por_cobrar").insert(payload);
      if (error) { toast("Error al crear la OC.", "error"); setSaving(false); return; }
      toast("OC registrada correctamente", "success");
      setModalNueva(false);
    }
  ...
  async function eliminarOC(id) {
    const ok = await confirm({ title: "¿Eliminar esta OC?", message: "Esta acción no se puede deshacer.", confirmLabel: "Sí, eliminar", confirmVariant: "danger" });
    if (!ok) return;
    const { error } = await supabase.from("ordenes_por_cobrar").delete().eq("id", id);
  ...
  async function marcarPagada(oc) {
    if (!fechaPagoReal) { toast("Selecciona la fecha de pago.", "warning"); return; }
    setSaving(true);
    const { error } = await supabase.from("ordenes_por_cobrar").update({ fecha_pago_real: fechaPagoReal, estado: "pagado" }).eq("id", oc.id);
    if (error) { toast("Error al registrar el cobro.", "error"); setSaving(false); return; }
    toast(`OC ${oc.numero_oc} marcada como cobrada — ${fmt(oc.monto_total)}`, "success");
    setModalPagar(null); setFechaPagoReal(""); loadData(); setSaving(false);
  }

  async function agregarCliente() {
    if (!nuevoCliente.nombre) { toast("Escribe el nombre del cliente.", "warning"); return; }
    setSaving(true);
    const { error } = await supabase.from("clientes_oc").insert(nuevoCliente);
    if (error) { toast("Error al agregar cliente.", "error"); setSaving(false); return; }
    toast(`Cliente "${nuevoCliente.nombre}" agregado`, "success");
    setNuevoCliente({ nombre: "", plazo_dias: 90 }); setModalCliente(false); loadData(); setSaving(false);
  }

  function abrirEditar(oc) {
    setForm({ cliente_id: oc.cliente_id, numero_oc: oc.numero_oc, descripcion: oc.descripcion || "", fecha_facturacion: oc.fecha_facturacion || "", monto_total: oc.monto_total || "", fecha_vencimiento: oc.fecha_vencimiento || "", fecha_pago_esperado: oc.fecha_pago_esperado || "", fecha_pago_real: oc.fecha_pago_real || "", notas: oc.notas || "" });
    setDocFile(null); setDocPreview(oc.documento_url || null); setModalEditar(oc);
  }
  ...
  const estadoStyle = { pagado: { bg: "#dcfce7", color: "#15803d", label: "✅ Cobrada" }, pendiente: { bg: "#fef9c3", color: "#854d0e", label: "⏳ Pendiente" }, vencido: { bg: "#fee2e2", color: "#991b1b", label: "🔴 Vencida" }, en_proceso: { bg: "#fff7ed", color: "#c2410c", label: "🚚 En proceso" } };
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Btn variant="secondary" onClick={() => setVistaCalendario(v => !v)}>{vistaCalendario ? "📋 Ver lista" : "📅 Ver calendario"}</Btn>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        ...
        {["all","en_proceso","pendiente","pagado","vencido"].map(f => (
          <button key={f} onClick={() => setFiltroEstado(f)}
            style={{ border: "none", borderRadius: 20, padding: "6px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", background: filtroEstado === f ? (f === "en_proceso" ? "#c2410c" : "#0f172a") : "#f1f5f9", color: filtroEstado === f ? "#fff" : "#64748b" }}>
            {f === "all" ? "Todas" : f === "en_proceso" ? "🚚 En proceso" : f === "pendiente" ? "⏳ Pendientes" : f === "pagado" ? "✅ Cobradas" : "🔴 Vencidas"}
          </button>
        ))}
      </div>

      {vistaCalendario ? <Calendario eventos={eventosCalendario} /> : (
        <>
          {isMobile ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {filtradas.map(oc => {
                const est = getEstadoReal(oc); const s = estadoStyle[est];
                return (
                  <div key={oc.id} style={{ background: "#fff", borderRadius: 14, padding: 14, border: "1.5px solid #e2e8f0", borderLeft: est === "en_proceso" ? "4px solid #f97316" : est === "vencido" ? "4px solid #ef4444" : "1.5px solid #e2e8f0" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                      <div><div style={{ fontWeight: 700, fontSize: 13, color: "#0f172a" }}>{oc.clientes_oc?.nombre}</div><div style={{ fontSize: 11, color: "#64748b", fontFamily: "monospace", marginTop: 2 }}>{oc.numero_oc}</div></div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                      <div style={{ background: "#f8fafc", borderRadius: 8, padding: "8px 10px" }}><div style={{ fontSize: 10, color: "#94a3b8", marginBottom: 2 }}>MONTO</div><div style={{ fontSize: 14, fontWeight: 700, color: "#0f172a" }}>{fmt(oc.monto_total)}</div></div>
                      <div style={{ background: "#f8fafc", borderRadius: 8, padding: "8px 10px" }}><div style={{ fontSize: 10, color: "#94a3b8", marginBottom: 2 }}>VENCIMIENTO</div><div style={{ fontSize: 12, fontWeight: 600, color: est === "vencido" ? "#dc2626" : "#0f172a" }}>{fmtDate(oc.fecha_vencimiento)}</div></div>
                    </div>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {oc.documento_url && <a href={oc.documento_url} target="_blank" rel="noreferrer"><Btn variant="info" style={{ padding: "5px 10px", fontSize: 11 }}>📎 Ver doc</Btn></a>}
                      {est !== "pagado" && <Btn variant="success" style={{ padding: "5px 10px", fontSize: 11 }} onClick={() => { setModalPagar(oc); setFechaPagoReal(new Date().toISOString().split("T")[0]); }}>💰 Cobrar</Btn>}
                      <Btn variant="info" style={{ padding: "5px 10px", fontSize: 11 }} onClick={() => abrirEditar(oc)}>✏️ Editar</Btn>
                      <Btn variant={oc.investment_order_id ? "success" : "purple"} style={{ padding: "5px 10px", fontSize: 11 }} onClick={() => oc.investment_order_id ? null : abrirCrearOrden(oc)} disabled={!!oc.investment_order_id}>
                        {oc.investment_order_id ? "✅ Orden creada" : "📋 Crear orden"}
                      </Btn>
              ...
              {filtradas.length === 0 && <div style={{ padding: 40, textAlign: "center", color: "#94a3b8", background: "#fff", borderRadius: 16 }}>No hay OC registradas</div>}
            </div>
          ) : (
            <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", overflow: "hidden" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                ...
                <tbody>
                  {filtradas.map((oc, i) => {
                    ...
                    return (
                      <tr key={oc.id} style={{ borderTop: "1px solid #f1f5f9", background: est === "en_proceso" ? "#fff7ed" : i % 2 === 0 ? "#fff" : "#fafbfc", borderLeft: est === "en_proceso" ? "3px solid #f97316" : "none" }}>
                        ...
                        <td style={{ padding: "11px 14px" }}><span style={{ background: s.bg, color: s.color, borderRadius: 20, padding: "3px 10px", fontSize: 11, fontWeight: 700 }}>{s.label}</span></td>
                        <td style={{ padding: "11px 14px" }}>
                          {oc.documento_url ? <a href={oc.documento_url} target="_blank" rel="noreferrer"><Btn variant="info" style={{ padding: "4px 10px", fontSize: 11 }}>📎 Ver</Btn></a> : <span style={{ fontSize: 11, color: "#cbd5e1" }}>—</span>}
                        </td>
                        <td style={{ padding: "11px 14px" }}>
                  ...
                  {filtradas.length === 0 && <tr><td colSpan={10} style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>No hay OC registradas</td></tr>}
                </tbody>
              </table>
            </div>
          )}
      ...
      <Modal open={!!modalPagar} onClose={() => setModalPagar(null)} title="Registrar cobro" maxWidth={400}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Btn variant="success" loading={saving} onClick={() => marcarPagada(modalPagar)} style={{ flex: 1 }}>✅ Confirmar cobro</Btn>
        </div>
      </Modal>
      ...
      <Modal open={!!modalCrearOrden} onClose={() => setModalCrearOrden(null)} title="Crear orden de inversión" maxWidth={520}>
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#64748b", marginBottom: 6, textTransform: "uppercase" }}>Imagen para la orden (opcional)</div>
          <div onClick={() => imgOrdenRef.current?.click()} style={{ border: "2px dashed #e2e8f0", borderRadius: 12, padding: imgOrdenPreview ? 4 : 16, textAlign: "center", cursor: "pointer", background: "#fafafa", minHeight: 70, display: "flex", alignItems: "center", justifyContent: "center" }}>
            {imgOrdenPreview ? <img src={imgOrdenPreview} alt="preview" style={{ maxHeight: 140, maxWidth: "100%", borderRadius: 8 }} />
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Btn loading={savingOrden} onClick={guardarOrdenDesdeOC} style={{ flex: 1, background: "#7c3aed", color: "#fff" }}>📋 Crear borrador</Btn>
        </div>
      </Modal>
      ...
      <Modal open={modalCliente} onClose={() => setModalCliente(false)} title="Agregar cliente" maxWidth={380}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Btn loading={saving} onClick={agregarCliente} style={{ flex: 1 }}>Agregar cliente</Btn>
        </div>
      </Modal>
    </div>
  );
...
function Nav({ perfil, tab, setTab, tabs, onLogout, accentColor }) {
  ...
  function handleTab(id) { setTab(id); setMenuAbierto(false); }

  const LogoSVG = () => (
    <svg width="32" height="32" viewBox="0 0 400 400" xmlns="http://www.w3.org/2000/svg" style={{ borderRadius: 8, flexShrink: 0 }}>
      <defs>
        <linearGradient id="navbg" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#1e40af"/><stop offset="100%" stopColor="#2563eb"/></linearGradient>
        <linearGradient id="navgold" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="#fbbf24"/><stop offset="100%" stopColor="#f59e0b"/></linearGradient>
      </defs>
  ...
  const showHamburger = isMobile || isTablet || tabs.length > 5;
    ...
    <>
      <div style={{ background: "#fff", borderBottom: "1.5px solid #e2e8f0", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 1px 8px rgba(0,0,0,0.06)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 20px", display: "flex", alignItems: "center", height: 60 }}>
          ...
          {/* TABS DESKTOP — solo si no es mobile ni tablet */}
          {!isMobile && !isTablet && (
            <div style={{ display: "flex", gap: 2, marginRight: 16 }}>
              {tabs.map(t => (
                <button key={t.id} onClick={() => handleTab(t.id)}
                  style={{ border: "none", background: tab === t.id ? "#eff6ff" : "transparent", color: tab === t.id ? "#2563eb" : "#64748b", borderRadius: 9, padding: "7px 13px", cursor: "pointer", fontSize: 12, fontWeight: tab === t.id ? 700 : 500, borderBottom: tab === t.id ? "2px solid #2563eb" : "2px solid transparent", transition: "all 0.15s", whiteSpace: "nowrap" }}>
                  {t.label}
                </button>
              ))}
            </div>
      ...
      {menuAbierto && (
        <div style={{ background: "#fff", borderBottom: "1.5px solid #e2e8f0", padding: "10px 20px 16px", boxShadow: "0 8px 24px rgba(0,0,0,0.1)", position: "sticky", top: 60, zIndex: 99 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "#f8fafc", borderRadius: 12, marginBottom: 10, border: "1.5px solid #e2e8f0" }}>
            <div style={{ width: 38, height: 38, borderRadius: 10, background: accentColor, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 800 }}>{perfil?.full_name?.[0] || "U"}</div>
            <div><div style={{ fontSize: 13, fontWeight: 700, color: "#0f172a" }}>{perfil?.full_name}</div><div style={{ fontSize: 11, color: "#94a3b8" }}>{perfil?.email}</div></div>
          </div>
          {tabs.map(t => (
            <button key={t.id} onClick={() => handleTab(t.id)}
              style={{ display: "flex", alignItems: "center", width: "100%", border: "none", background: tab === t.id ? "#eff6ff" : "transparent", color: tab === t.id ? "#2563eb" : "#374151", borderRadius: 10, padding: "12px 14px", cursor: "pointer", fontSize: 14, fontWeight: tab === t.id ? 700 : 400, marginBottom: 3, textAlign: "left", borderLeft: tab === t.id ? "3px solid #2563eb" : "3px solid transparent" }}>
              {t.label}
          ...
          <div style={{ borderTop: "1.5px solid #f1f5f9", marginTop: 10, paddingTop: 10 }}>
            <button onClick={onLogout} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", border: "1.5px solid #fecaca", background: "#fef2f2", color: "#dc2626", borderRadius: 10, padding: "11px 14px", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>🚪 Cerrar sesión</button>
          </div>
        </div>
      )}
    </>
...
function InvestorView({ perfil, onLogout }) {
  ...
  function handleTabChange(newTab) { setTab(newTab); if (newTab === "dashboard" || newTab === "inversiones") setReloadCount(c => c + 1); }
  const tabs = [
...
export default function App() {
  ...
  return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", color: "#94a3b8", fontFamily: "'DM Sans',sans-serif" }}>Rol no reconocido: {perfil.role}</div>;
}
